...
import type { CreateWorktreeOptions } from "./GitVcsDriver.ts";
...
export interface WorktreeFanoutTarget {
  ...
  /** Branch name `baseRef` was resolved from, when it is a pinned commit. */
  readonly baseRefName?: string;
  /** Passed through to `createWorktree` (progress and claim callbacks). */
  readonly options?: CreateWorktreeOptions;
}
          ...
          const result = yield* deps.createWorktree(
            {
              ...
              ...(target.baseRefName ? { baseRefName: target.baseRefName } : {}),
              path: worktreePath,
            },
            target.options,
          );
