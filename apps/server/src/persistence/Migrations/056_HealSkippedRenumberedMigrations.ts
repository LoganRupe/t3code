 ...
 *   is every machine that ran the branch between the first two rebases.
 * - 041 (AuthSessionClientConnection) was skipped on machines that ran the build
 *   numbering multi-repo 041-045. `auth_sessions` ends up missing
 *   `client_surface` and `client_app_version`, which the connection handler
 *   writes on every WebSocket connect.
 * - 042-043 (ThreadLinkedPullRequest/ThreadsUnsettledAt) were skipped on machines
 ...
 * - 050-051 (ThreadPullRequests/ThreadMessageContext) were skipped on machines
 ...
 * behavior. Healthy databases have everything already and this whole migration
 * is a no-op.
 *
