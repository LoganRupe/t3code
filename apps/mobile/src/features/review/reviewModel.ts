...
function buildGitSectionItems(
  ...
  return kinds.flatMap<ReviewSectionItem>((kind) => {
    ...
    // Per-file lazy loading and file stats address one cwd, so only the flat
    // single-repo source carries them; a grouped section renders its previews.
    const flatSource = groups.length === 0 ? first.source : null;
    return [
      {
        id: `git:${kind}`,
        kind,
        ...
        ...(flatSource?.files ? { files: flatSource.files } : {}),
        truncated: slices.some((slice) => slice.source.truncated),
        isLoading: false,
      },
    ];
  });
